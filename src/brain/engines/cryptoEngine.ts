...
export const cryptoEngine: BrainEngine = {
  name: "crypto",
  async run(ctx: BrainTickerContext): Promise<EngineSignalResult> {
    if (ctx.assetClass !== "crypto") return { engine: "crypto", tickerId: ctx.tickerId, score: null, bias: "unknown", missing: true, metadata: {} };
    if (ctx.priceData.length < 7) return { engine: "crypto", tickerId: ctx.tickerId, score: 50, bias: "neutral", missing: false, metadata: {} };
