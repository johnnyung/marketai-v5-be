...
export const insiderEngine: BrainEngine = {
  name: "insider",
  async run(ctx: BrainTickerContext): Promise<EngineSignalResult> {
    if (ctx.insider.length === 0) return { engine: "insider", tickerId: ctx.tickerId, score: 50, bias: "neutral", missing: true, metadata: {} };
    
    let buys = 0;
    let sells = 0;
    ctx.insider.forEach(t => {
      const type = t.transactionType?.toLowerCase() || "";
      if (type.includes("buy") || type.includes("purchase")) buys++;
      else sells++;
    });
