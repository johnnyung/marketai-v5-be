...
export const opportunityEngine: BrainEngine = {
  name: "opportunity",
  async run(ctx: BrainTickerContext): Promise<EngineSignalResult> {
    if (ctx.priceData.length < 5) return { engine: "opportunity", tickerId: ctx.tickerId, score: 50, bias: "neutral", missing: true, metadata: {} };
