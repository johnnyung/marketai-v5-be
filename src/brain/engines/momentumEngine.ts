...
export const momentumEngine: BrainEngine = {
  name: "momentum",
  async run(ctx: BrainTickerContext): Promise<EngineSignalResult> {
    if (ctx.priceData.length < 10) return { engine: "momentum", tickerId: ctx.tickerId, score: 50, bias: "neutral", missing: true, metadata: {} };
